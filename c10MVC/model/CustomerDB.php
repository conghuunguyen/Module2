...
class CustomerDB
{
    ...
    {
        $sql = "INSERT INTO customernew(name, email, address) VALUES (?, ?, ?)";
    ...
    public function get($id){
        $sql = "SELECT * FROM customernew WHERE id = ?";
        $statement = $this->connection->prepare($sql);
    ...
    public function delete($id){
        $sql = "DELETE FROM customernew WHERE id = ?";
        $statement = $this->connection->prepare($sql);
        $statement->bindParam(1, $id);
        return $statement->execute();
    }
    ...
    {
        $sql = "UPDATE customernew SET name = ?, email = ?, address = ? WHERE id = ?";
        $statement = $this->connection->prepare($sql);
        $statement->bindParam(1, $customer->name);
        $statement->bindParam(2, $customer->email);
        $statement->bindParam(3, $customer->address);
